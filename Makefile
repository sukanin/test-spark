...
run: ## Build the container without caching
	docker run --mount type=bind,source=$(CURDIR),target=/opt/application $(IMAGE_NAME) --packages org.apache.spark:spark-streaming-kafka-0-8_2.11:2.4.7 driver local:///opt/application/main.py -iz localhost:2181 -it input -oz localhost:2181 -ot output
